        ...
        <div v-loading="importing" style="margin-top: 30px; max-width: 1160px;"
            ...
            <template v-if="has_campaign_pro">
                <div class="fc_funnel_upload text-align-center">
                    <h3>{{ $t('Import_Recurring_Campaign') }}</h3>
                    <el-upload
                        drag
                        :limit="1"
                        :action="url"
                    ...
                    <h4>{{ $t('Not_Import_Recurring_Campaigns_Alert') }}</h4>
...
export default {
    ...
    computed: {
        url() {
            ...
            url += (url.match(/\?/) ? '&' : '?') + jQuery.param({ action: 'fluentcrm_import_recurring_campaigns' });
            return url;
        }
    },
    methods: {
