        ...
        <div v-loading="importing" style="margin-top: 30px; max-width: 1160px;"
            ...
            <template v-if="has_campaign_pro">
                <div class="fc_funnel_upload text-align-center">
                    <h3>{{ $t('Import_Dynamic_Segment') }}</h3>
                    <el-upload
                        drag
                        :limit="1"
                        :action="url"
                    ...
                    <h4>{{ $t('Not_Import_Dynamic_Segments_Alert') }}</h4>
...
export default {
    ...
    computed: {
        url() {
            ...
            url += (url.match(/\?/) ? '&' : '?') + jQuery.param({ action: 'fluent_crm_import_dynamic_segment' });
            return url;
        }
    },
    methods: {
