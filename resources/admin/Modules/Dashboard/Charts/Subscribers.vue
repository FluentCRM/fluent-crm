...
export default {
    ...
    methods: {
        fetchReport() {
            this.fetching = true;
            this.$get('reports/subscribers', {date_range: this.date_range})
                .then(response => {
        ...
        setupChartItems() {
            ...
            const monthYearFormat = /^[A-Za-z]{3} \d{4}$/;
            function isMonthYearFormat(labels) {
                return labels.every(label => monthYearFormat.test(label));
            }
            if (!isMonthYearFormat(labels)) {
                labels.sort();
            }
